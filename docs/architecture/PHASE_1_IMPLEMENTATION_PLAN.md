...
| T-13 | **§17.4's mandatory precision test produced its designated failure: linear FP16 does not hold at low amplitude.** Ordinary content is unaffected — worst relative error 4.860e-04 against the 4.9e-4 FP16 step, and 100.00% significant-bin survival on sine, drums, white-noise and clipping fixtures. The very-quiet fixture (−120 dBFS) survives only **4.94%**, because binary16's absolute floor is ~−144 dBFS and a −120 dBFS spectrum lands in the subnormal range. §17.4 documents the fallback — a **dB-domain variant** — and states that adopting it is a **format change requiring a `formatVersion` bump**. Measured, recorded in `PERFORMANCE.md` §1.3, and **not applied**: it is a normative format decision, not an implementation choice. **Not recoverable** once tier-3 files exist in the field. | §17.4, `PERFORMANCE.md` §1.3, `AnalysisCacheFormat.FORMAT_VERSION` | **Before any tier-3 cache file is written on a user device** |
| T-14 | **§21 beat detection is entirely unspecified — this blocks stage 4 and `audio:beat`.** §21 requires beats be detected against "a documented threshold", but no threshold value is documented anywhere in the specification; no beat-detection algorithm is named; and `beatConfidence`, which §17 lists among the required features, has no definition. None of the three was invented, so **stage 4 and `audio:beat` were not implemented in Step 10** (see the Step 10 report). Separately, `BeatConfig.minTempoBpm` and `BeatConfig.maxTempoBpm` are **unratified Step 2 inventions that currently participate in `analysisConfigHash`** — the same defect as `NormalizationConfig.targetLufs`, which D-5 removed. Whether they stay is part of the same decision, and changing it changes every hash value. | §21, §17, `BeatConfig`, `audio:beat` | **Blocking — required before stage 4 / `audio:beat` can be built** |
...
10. `audio:analysis` stages 3–5 ✅ — `audio:beat` **not built**, blocked on **T-14** (§21 documents no threshold, names no algorithm, and leaves `beatConfidence` undefined)
