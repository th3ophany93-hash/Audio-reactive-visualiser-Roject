...
| T-15 | **§21.1 names a beat event's `strength` but gives it no formula.** The other three fields are defined exactly (`timestamp`, `frameIndex`, and `confidence` by its clamp expression); `strength` is required to exist and left open. Implemented as the raw onset value at the beat — `flux[frameIndex]`, un-normalized, which is the quantity `confidence` is the threshold-relative normalisation *of*. Stated at the definition site, not silently chosen. **Recoverable** — the flux series is a stored §17 feature, so a different definition is a read-time recomputation with no re-analysis. | §21.1, `BeatEvent.strength` | Before beat strength is exposed to §23 reactive mappings |
| T-16 | **§17.5's zero-padded tail produces a spurious high-confidence onset at the end of every track.** A frame whose 2048-sample window runs past the end of the signal contains a hard truncation, which smears energy across the spectrum and reads as a large positive flux under §21.1's definition. Measured on the §119 sine fixture: flux 0.653 on frame 197 of 200, against 9.8e-05 through the sustained portion — so the artefact is not marginal, it is the largest onset in the track and lands at `confidence = 1.0`. The affected frames are the final `ceil(windowSamples/hopSamples) − 1` (four, at the canonical framing). **Not invented away.** Excluding tail frames from beat detection, or windowing the tail differently, is a normative decision §21.1 does not make and §17.5 does not anticipate; the alternative — that a phantom beat fires at the end of every song — is a real user-visible defect. Needs a decision. | §17.5, §21.1, `BeatStage`, `BeatDetector` | **Before beat-driven mappings ship** |
